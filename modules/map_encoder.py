...
class MapEncoder(nn.Module):
    ...
    def forward(self, data: Batch) -> torch.Tensor:
        ...
        if data['lane']['num_nodes'] == 0:
            return torch.zeros(0, self.hidden_dim, device=data['agent']['position'].device)
        ...
        l_input = torch.stack([l_length, l_is_intersection.float(), l_turn_direction.float(), l_traffic_control.float()], dim=-1)  # [(M1,...,Mb),4]
        ...
        if c2l_edge_index.size(1) == 0:
            # No edges, return lane embeddings directly
            return l_embs
